...
const int maxIterationsRef =
	32; // Higher numbers are always more accurate, but if I can get away with it, I'll drop this to 16.
...
const float stopThreshold =
	0.01; // I'm not quite sure why, but thresholds in the order of a pixel seem to work better for me... most times.
...
float sinusoidalBumps ( in vec3 p ) {
	...
	return sin ( p.x * 4. + time * 0.97 ) * cos ( p.y * 4. + time * 2.17 ) * sin ( p.z * 4. - time * 1.31 ) + 0.5 * sin (
			   p.x * 8. + time * 0.57 ) * cos ( p.y * 8. + time * 2.11 ) * sin ( p.z * 8. - time * 1.23 );
}
...
float cheapAnimateVoronoi ( vec3 uv ) {
	...
	float d = 1.36; // The highest possible maximum is sqrt(3), by the way.
...
float rayMarching ( vec3 origin, vec3 dir, float start, float end ) {
	...
	float sceneDist = 1e4;
	float rayDepth =
		start; // Ray depth. "start" is usually zero, but for various reasons, you may wish to start the ray further away from the origin.
		
	for ( int i = 0; i < maxIterations; i++ ) {
		sceneDist = scene ( origin + dir *
							rayDepth ); // Distance from the point along the ray to the nearest surface point in the scene.
...
vec3 lighting ( vec3 sp, vec3 camPos, int reflectionPass ) {
	...
	if ( reflectionPass == 0 ) surfNormal = doBumpMap ( sp, surfNormal,
												0.04 ); //Bump factor changes with detail. Usually, the more detail an image has, the less bump it will need.
	...
	if ( reflectionPass == 0 ) shadowcol = softShadow ( sp, ld, stopThreshold * 2.0, len,
											   16.0 ); // Be careful not to let the ray hit the object upon initiation.
	...
	specular = pow ( specular,
					 specularPower ); // Ramping up the specular value to the specular power for a bit of shininess.
...
void main ( void ) {
	...
	vec3 camPos = vec3 ( 0. + 3.25 * cos ( time * 0.125 ), 2.,
						 2.5 + 3.25 * sin ( time *
											0.125 ) ); // This is the point you look from, or camera you look at the scene through. Whichever way you wish to look at it.
	...
	vec3 right = normalize ( vec3 ( forward.z, 0.,
									-forward.x ) ); // Right vector... or is it left? Either way, so long as the correct-facing up-vector is produced.
	...
	float refCoef =
		0.35; // Reflective coefficient. The amount of reflected light we wish to incorporate into the final color.
