...
// Last update: 11th Sep, 2014

// WebGL code containing a shader script to ray march a distorted spherical object and floor with shadows. Cheap 
// displacement mapping provides the majority of the distortion, and the finer surface detail is acheived via 
// bump mapping, using a Voronoi function. In this particular example, reflection functionality has been disabled - 
// because it's a little expensive and doesn't really add to the overall effect. You could define it back in, 
// if you like, but you'd have to make some setting changes (less distortion, bump, etc) to get it looking right.
//
// I've noticed that the framerate on this isn't the best. I know of a few ways to speed it up, so I'll make those
// changes in due course. I faster Voronoi funciton wouldn't hurt, but I have one of those somewhere, so I'll dig
// it up.
//
